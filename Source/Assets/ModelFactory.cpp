...
Model* ModelFactory::BuildModel(std::string fileName) 
{
	...
	if (!obj.is_open())
	{
		Logger::Debug("--ERROR-- Failed to load model");
		return nullptr;
	}
	...
	GraphicsManager::Get().GetDevice()->CreateBuffer(&vbd, &initialVertexData, model->GetVertexBuffer());

	// Create Index Buffer Description
	D3D11_BUFFER_DESC ibd;
	ibd.Usage = D3D11_USAGE_IMMUTABLE;
	ibd.ByteWidth = sizeof(int) * verts.size();
	...
	initialIndexData.pSysMem = (int*)&indices[0];

	// Create Index Buffer
	GraphicsManager::Get().GetDevice()->CreateBuffer(&ibd, &initialIndexData, model->GetIndexBuffer());
